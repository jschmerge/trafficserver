...
build_vol_hash_table(CacheHostRecord *cp)
{
  ...
  num_vols -= bad_vols;
  Debug("amc", "Rebuilding stripe assignment - using %d of %d stripes", num_vols, num_vols + bad_vols);
  ...
  // install new table
  if (0 != (old_table = static_cast<unsigned short*>(ink_atomic_swap_ptr(&(cp->vol_hash_table), ttable))))
    ...
    for (; config_vol; config_vol = config_vol->link.next) {
      ...
      if (config_vol->size < 128) {
        Warning("the size of volume %d (%"PRId64") is less than the minimum required volume size %d",
                config_vol->number, (long)config_vol->size, 128);
        Warning("volume %d is not created", config_vol->number);
      }
      Debug("cache_hosting", "Volume: %d Size: %"PRId64, config_vol->number, (long)config_vol->size);
    }
    cplist_update();
...
Cache::key_to_vol(CacheKey *key, char *hostname, int host_len)
{
  ...
  if (hash_table) {
    ...
    Debug("amc", "key to vol - #%d -> %d", h, hash_table[h]);
